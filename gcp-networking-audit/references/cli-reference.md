...
Read-only gcloud CLI commands organised by audit step for VPC Network assessment. All commands are non-modifying (`list`, `describe`, `get-status`, `get-iam-policy`, `get-value`). No command creates, modifies, or deletes resources. See `../SKILL.md` for the six-step audit procedure these commands support.

Access method: gcloud CLI with an authenticated account. Commands assume `--project <project-id>` is set via `gcloud config set project` or appended to each command. Output defaults to YAML; append `--format=table` for human-readable output during interactive audits.
...
| Subnets in a specific network | `gcloud compute networks subnets list --network <network>` |
...
| Port utilisation | `gcloud monitoring metrics list --filter="metric.type=router.googleapis.com/nat/open_connections"` |
...
| List Shared VPC organisations | `gcloud compute shared-vpc organizations list` |
...
| Function | Command |
|----------|---------|
| List peerings on a network | `gcloud compute networks peerings list --network <network>` |
...
- Cloud Router `get-status` returns learned routes, advertised routes, and BGP peer state. This single command gives the most comprehensive routing view.
- A route whose `nextHopIp` points to a stopped VM creates silent packet drops (the GCP analogue of an AWS black-hole route).
- GCP uses a route priority system (0 to 65535, default 1000) for custom routes. Lower number = higher preference.
- The VPC Network routing mode (regional vs global) determines whether Cloud Router learned routes propagate across regions.
...
| Organisation ID | `gcloud organizations list` |
