...
- **Validating a third-party vendor's pentest report before acting on it**: use `verify-security-findings`. This skill is for running an engagement; that one is for re-verifying someone else's findings.
...
**Written authorisation is the precondition, not a formality.** No active technique against any target proceeds without confirmed written authorisation naming the scope, the time window, the allowed techniques, and an escalation contact. This applies equally to an external consultant, an internal red team, and this skill itself: refusing to proceed without confirmation is the correct behaviour, not excessive caution. See `~/.claude` project-level guidance for where an organisation's own authorisation record should live (for Kacific specifically, `kacific-security-assessment-scope` names the pointer).
...
- `verify-security-findings`: apply this discipline before finalising the report if any finding originated from an automated scan rather than manual validation.
