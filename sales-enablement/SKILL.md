...
> **Skill marker**: When applying this skill, begin your reply with `[skill: sales-enablement]` on its own line so the transcript shows the skill fired. If multiple skills fire on the same reply, emit each marker on its own line at the top: transparency over neatness.

## Initial Assessment

If a `CLAUDE.md` or `AGENTS.md` exists in the working directory, read it first to understand the brand voice, value proposition, and sales motion before drafting collateral. Only ask the user for information not already covered or that is specific to this asset.
...
### Customisation by Buyer Type

| Buyer | Emphasise | De-emphasise |
|-------|-----------|--------------|
...
- **Spreadsheet**: Fastest to build, easy to customise per deal. Works for inside sales.
...
2. **Discovery recap** (3 min): Summarise what you learned, confirm priorities
...
5. **Close** (5 min): Summarise value, propose next steps with timeline
...
- **Customise to their use case.** Use their terminology, their data (if possible), their workflow.
...
Organise case studies so reps can find the right one instantly:
...
- **Too generic**: Templated proposals signal low effort. Customise the exec summary at minimum.
...
- **Discovery questions**: Organised by topic, not a script
...
- **Blocker**: Opposes the purchase. Understand their concern to neutralise it.
...
| Proposal | Section-by-section copy with customisation notes |
...
- `competitor-alternatives`: public-facing comparison and alternative pages; battle cards for sales reps draw from the same competitive research.
- `competitor-profiling`: structured competitor research that feeds the differentiation slides, objection-handling responses, and battle cards.
- `copywriting`: marketing website copy that the deck and one-pager point at; voice and value-prop should match across both surfaces.
- `cold-email`: outbound prospecting; sales-enablement collateral is what the cold-email reply chain hands off to.
- `email-sequence`: lifecycle and nurture flows that link to one-pagers, case studies, and ROI calculators built here.
- `revops`: lead lifecycle, scoring, routing, and pipeline management; revops defines the stage at which each piece of collateral is needed.
- `pricing-strategy`: pricing decisions and packaging that the pricing slide and proposal section embed; sales-enablement must reflect current pricing.
- `customer-research`: ICP, persona, and pain-point evidence that anchors the deck story, the one-pager problem statement, and the objection-handling doc.
- `marketing-psychology`: persuasion, anchoring, social proof, and framing applied to deck slides, ROI displays, and proposal exec summaries.
- `humanise-comms`: voice and tone discipline for human-bound sales communications (proposal cover notes, follow-up emails, executive summaries).
