...
- **`/system script add source="..."` on a single command line eagerly expands bare `$variable` references at CLI-PARSE time**, before the script body is ever stored, so every `$var` inside the source silently becomes empty. This bites hardest when adding a script programmatically via a one-shot exec-style command (SSH `exec_command`, a REST call building the command string, etc.), since there is no interactive prompt to catch the substitution happening. Escape every `$` as `\$` in the source before embedding it in the `add` command line, then read the stored script back (`/system script print detail`) to confirm each `$` survived, rather than trusting the add to have worked.
