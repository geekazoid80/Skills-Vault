...
The precondition above is what a brief must INCLUDE. This is the other end: what a brief FORBIDS. A "do not touch X" fence is cheap to write and its cost is invisible to the author, because the author is not the one who hits it. Fences exist for good reasons - a dated record that must not be rewritten, a peer's in-flight surface, a rule the operator has settled - so the defect is never fencing itself. It is fencing WITHOUT saying what happens when the change runs into the fence.

When the work you are briefing might have to reconcile with a fenced line, the fence MUST carry its own escape hatch. Three usable dispositions, any of which beats silence:

1. **Reconcile in place anyway** - the fence is about content, not existence (e.g. "do not rewrite the dated incident, but you may add a pointer line").
2. **Reconcile one way and raise the other half back** to a NAMED place (the tracking task, the operator, a follow-up brief).
3. **Stop and ask.**

Silence forces the receiving session to choose between disobeying the fence and shipping something half-connected: a new rule that points at the fenced line while the fenced line still points nowhere, so a reader who stops at the fenced line meets the old advice with no hint it was scoped. That is the failure this rule exists to prevent, and closing it costs a second PR.
...
- A brief that FENCES a line ("do not touch X") the change must reconcile with, WITHOUT saying what to do if the work runs into the fence (reconcile anyway / do half and escalate to a named place / stop and ask). Silence forces the receiving session to choose between disobeying the fence and shipping a half-connected result.
