...
- **Safe patterns table** (do NOT flag): `pull_request_target` WITHOUT fork checkout, numeric IDs, `${{ secrets.* }}`, `${{ }}` in `if:` / `with:` / job-level `env:`, full-SHA pinning, `pull_request` trigger.
