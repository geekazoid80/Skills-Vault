...
Read-only AWS CLI commands organised by audit step for VPC networking assessment. All commands are non-modifying (`describe`, `get`, `list`, `search`, `filter`). No command creates, modifies, or deletes resources.

Access method: AWS CLI v2 with configured credentials. Commands assume `--region <region>` is set via environment variable or appended to each command. Output format defaults to JSON; append `--output table` for human-readable output during interactive audits.

Pairs with the parent skill: see `../SKILL.md` for the six-step audit procedure that uses these commands.
...
- The local route (VPC CIDR to local) cannot be removed and handles intra-VPC routing.
...
### Credential probe (secrets-hygiene pattern)

Per the parent skill's Prerequisites section and the `secrets-hygiene` "Probing the credential store" subsection: probe for credential presence with exit-code-only checks; never print or pipe credentials to stderr / stdout.

```
aws sts get-caller-identity > /dev/null 2>&1 && echo "credentials: ok" || echo "credentials: MISSING"
```

Never use `aws sts get-caller-identity | tee /dev/stderr` or any pattern that surfaces the account / ARN / user-id values into the chat transcript when those values themselves are sensitive (production account IDs may be classified per your org's policy).
