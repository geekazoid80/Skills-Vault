...
2. **Pin third-party actions by SHA, not by tag.** A tag is mutable; a third-party action's owner (or an attacker who compromises them) can retarget it, as happened to `tj-actions/changed-files` in March 2025 (CVE-2025-30066, tags repointed to code that leaked CI secrets). Pin as `uses: tj-actions/changed-files@<40-char-sha> # v46` so it cannot be silently retargeted; Dependabot will PR the SHA bumps. First-party `actions/*` and `github/*` (e.g. `actions/checkout@v4`) may keep a major-version tag, since GitHub controls those namespaces. See `secrets-hygiene` for the full rule.
