...
- Leaving your own worktree behind on the assumption that the harness, the session ending, or archiving the session will clear it. None of them does (see Layer 4 Step 5 and `using-git-worktrees`).
...
- **Worktree, if applicable.** **Do not assume something else disposes of it.** This step previously said the harness owns cleanup for a path under `.claude/worktrees/` and would clear it when the session ended. That is false and it is the reason worktree residue accumulates: ending a session does not remove a worktree, archiving one does not either, and the native exit helper only handles worktrees it created in the current session. So the owning session removes its own, and `using-git-worktrees` carries the full disposal rule including why it must be the last tool call of a session that is closing. Use `git worktree remove` from the canonical repo root (not from inside the worktree, whose directory is about to vanish), then `git worktree prune` to clear the registration. Push first, and pin any commit that exists only locally to `refs/archive/<date>/<name>` before removing, since a SHA in a transcript does not survive it. **Never remove a worktree this session did not create**; a peer's live workspace is indistinguishable from residue from outside. If `git worktree remove` is refused (`contains modified or untracked files`), the worktree holds files that exist nowhere else, uncommitted plans, notes, or scratch work. Do NOT `--force` on your own initiative. Show what is at stake (`git -C "$WORKTREE_PATH" status --porcelain -uall`) and ask which of three: commit them to the branch before cleanup, move them into the main repo root, or delete them (unrecoverable). Carry out the choice, then remove the worktree.
...
| 5 | Switch off branch; `git branch -D <branch>`; remove your OWN worktree yourself (nothing else does it) without `--force`, from the canonical root; `git fetch --prune` |
