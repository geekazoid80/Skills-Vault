...
**Confirmed 2026-09-19** (Kacific `graylog_ncc_cyberaudit_admin_token`, reachable only from an internal host): reproduced deterministically with throwaway values (`echo "$P:t"` where `P=/usr/local/bin` printed `bin`, the zsh `:t` modifier working exactly as designed), isolating the mechanism to plain shell text substitution — it fired identically for a bare `echo`/`printf` with no curl or network involved at all — and confirming it is unrelated to curl, to the network, or to the specific host. Both the original exposure and its recurrence were genuine, full, unredacted exposures of the live token value, not a network fluke or a redaction-decoy artefact; the credential was rotated once the mechanism was understood. Full trail, including the diagnostic method (test with a disposable/fake credential first, never the live one, to keep the diagnosis itself from being a new leak): `~/.claude/memory/transcript_secret_leak_paths.md` Path 7, and the underlying shell mechanism: the public Skills-Vault sibling gotcha, `feedback_bash_tool_zsh_wordsplit.md` gotcha (9), which had already documented the modifier family generically (via a `git show "$SHA:path"` misread) before this recurrence gave it a credential-exposure instance.
