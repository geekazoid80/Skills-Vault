...
**Elimination from a list is not proof, even a careful one with a documented difference backing it.** A found credential narrowed down to two or three live candidates by digest comparison, expiry, and account identity is still narrowed by inference, not confirmed. A capability check (does this one have write access, does it read as one thing or another) is tempting to use as the final discriminator, and it can be actively wrong when more than one candidate shares the capability being tested, or when a permission field describes the resource owner rather than the specific credential (see *What can this credential actually do?* below for the same trap in a different shape).

**Dated origin, 2026-09.** A found credential was narrowed to two live candidates by digest comparison against every known store. A capability probe pointed at one of them; deleting that one and re-testing the found value showed it was STILL live, meaning the probe's signal had been backwards for this pair. Only deleting the other candidate and re-testing killed it. Deleting the wrong candidate first was not free either: it happened to be a live credential something else genuinely depended on, so a wrong guess here has a real cost, not just a wasted step.

**Delete-and-retest is the only fully definitive confirmation once you are down to inference between live candidates.** It costs nothing extra when the candidate needs rotating regardless of which one turns out to be the match, which is the common case once you are already at "found a credential, do not know its origin, need to revoke it". Order candidates by your best evidence rather than arbitrarily, because a miss on the wrong one has its own blast radius: confirm what depends on a candidate before deleting it as a test, not only after.

**When the credential is scoped to an organisation, check the organisation's own credential-management page, not only the individual account's.** Most platforms with org-scoped fine-grained tokens (GitHub's fine-grained personal access tokens are one instance) expose an org-level view listing every token with access to that org's resources, from every identity that holds one, not just your own. An individual's own token list is a strict subset and can miss tokens issued by service accounts or other admins entirely; the org-level list is the more complete and authoritative one to search when the resource in question is org-owned.
...
## Brace every variable expansion followed by a literal colon, in zsh

A shell running **zsh** (not bash) reads `:` immediately after a **bare, unbraced** `$VAR` as a history-style **modifier** flag (`:t` tail/basename, `:h` head/dirname, `:r` root, `:e` extension, `:s`, `:g`, `:l`, `:u`, `:q`, `:a`, ...), even inside double quotes. `"$TOK:token"` is parsed as "$TOK, then apply the `:t` modifier", which silently consumes exactly the two characters `:t` and leaves the literal remainder (`oken`) appended directly to the unmodified value, with the separating colon gone entirely.

**Why this is a credentials issue, not just a correctness one.** `curl -u "$TOK:token"` under this bug hands curl a single argument with no colon in it at all. Curl's real, documented behaviour for `-u user` with no password is to fall back to an interactive password prompt (`Enter host password for user '<value>':`), and it prints that prompt, **with the value verbatim**, to whatever is capturing the command's output. If that output is a persisted transcript (an agent tool result, a CI log, a session recording), the live credential is now sitting in it, unredacted, under a message that looks like an unrelated SSH/network glitch rather than a credential leak. Nothing about the command errors in a way that flags itself as a secrets problem; it just looks like flaky connectivity.

**The fix is not a letter list, it's a habit.** Brace every expansion that is followed by a literal colon: `"${TOK}:token"`, `"${SHA}:path"`, `"${F}:h"`. This costs nothing when the following letter would have been harmless and closes the hole when it would not. Never rely on remembering which single letters are modifiers (`t`, `h`, `r`, `e`, `s`, `g`, `l`, `u`, `q`, `a`, ...); any of them can appear as the first letter of an ordinary literal suffix.

**Confirmed 2026-09-19** (Kacific `graylog_ncc_cyberaudit_admin_token`, reachable only from an internal host): reproduced deterministically with throwaway values (`echo "$P:t"` where `P=/usr/local/bin` printed `bin`, the zsh `:t` modifier working exactly as designed), isolating the mechanism to plain shell text substitution (it fired identically for a bare `echo`/`printf` with no curl or network involved at all), confirming it is unrelated to curl, to the network, or to the specific host. Both the original exposure and its recurrence were genuine, full, unredacted exposures of the live token value, not a network fluke or a redaction-decoy artefact; the credential was rotated once the mechanism was understood. Full trail, including the diagnostic method (test with a disposable/fake credential first, never the live one, to keep the diagnosis itself from being a new leak): `~/.claude/memory/transcript_secret_leak_paths.md` Path 7, and the underlying shell mechanism: the public Skills-Vault sibling gotcha, `feedback_bash_tool_zsh_wordsplit.md` gotcha (9), which had already documented the modifier family generically (via a `git show "$SHA:path"` misread) before this recurrence gave it a credential-exposure instance.
