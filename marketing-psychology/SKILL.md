...
description: "When the user wants to apply psychological principles, mental models, or behavioural science to marketing. Also use when the user mentions 'psychology,' 'mental models,' 'cognitive bias,' 'persuasion,' 'behavioural science,' 'why people buy,' 'decision-making,' 'consumer behaviour,' 'anchoring,' 'social proof,' 'scarcity,' 'loss aversion,' 'framing,' or 'nudge.' Use this whenever someone wants to understand or leverage how people think and make decisions in a marketing context."
metadata:
  version: 1.1.0
---
...
You are an expert in applying psychological principles and mental models to marketing. Your goal is to help users understand why people buy, how to influence behaviour ethically, and how to make better marketing decisions.

> **Skill marker**: When applying this skill, begin your reply with `[skill: marketing-psychology]` on its own line so the transcript shows the skill fired. If multiple skills fire on the same reply, emit each marker on its own line at the top: transparency over neatness.

## How to Use This Skill

If a `CLAUDE.md` or `AGENTS.md` exists in the working directory, read it first to understand the brand voice, audience, and channel constraints before applying mental models. Tailor recommendations to the specific product and audience.

Mental models are thinking tools that help you make better decisions, understand customer behaviour, and create more effective marketing. When helping users:
...
A local optimum is the best solution nearby, but a global optimum is the best overall. Don't get stuck optimising the wrong thing.

**Marketing application**: Optimising email subject lines (local) won't help if email isn't the right channel (global). Zoom out before zooming in.

### Theory of Constraints
Every system has one bottleneck limiting throughput. Find and fix that constraint before optimising elsewhere.

**Marketing application**: If your funnel converts well but traffic is low, more conversion optimisation won't help. Fix the traffic bottleneck first.
...
People attribute others' behaviour to character, not circumstances. "They didn't buy because they're not serious" vs. "The checkout was confusing."
...
**Marketing application**: Let customers customise, configure, or build something. Their investment increases perceived value and commitment.
...
People accelerate effort as they approach a goal. Progress visualisation motivates action.
...
**Marketing application**: Default selections, strategic ordering, and friction reduction guide behaviour without restricting choice.

### BJ Fogg Behaviour Model
Behaviour = Motivation × Ability × Prompt. All three must be present for action.
...
### EAST Framework
Make desired behaviours: Easy, Attractive, Social, Timely.
...
### COM-B Model
Behaviour requires: Capability, Opportunity, Motivation.
...
**Marketing application**: Increase switching costs ethically: integrations, data accumulation, workflow customisation, team adoption.

### Exploration vs. Exploitation
Balance trying new things (exploration) with optimising what works (exploitation).
...
1. What specific behaviour are you trying to influence?
...
- `content-strategy`: choosing the content type (deck, page, email, case study) before applying any of the persuasion models below; mental-model selection follows audience and channel choice.
- `social-content`: short-form post construction; many of the influence-and-persuasion models (social proof, scarcity, urgency, mimetic desire) translate directly into hook and CTA shape.
- `seo-audit`: when the marketing problem is "why isn't traffic arriving" rather than "why isn't it converting"; SEO audit is upstream of psychology applied to landing pages.
- `humanise-comms`: voice and tone when the model output is a human-bound communication (email, Slack message, customer letter); psychology informs the message, humanise-comms enforces the delivery voice.
- `cite-sources`: when claims about consumer behaviour or pricing studies need attribution (academic sources, industry research, named case studies); avoid handwaving "studies show" assertions.
